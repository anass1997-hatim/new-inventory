...
const BarcodeOrQR = ({ type, value, width, height, format, attributes = [] }) => {
    const barcodeRef = useRef();
    const qrRef = useRef();
    const attributeLabels = useMemo(
        () => (attributes.length ? attributes.map((a) => a.label).join(" ") : ""),
        [attributes]
    );

    const scaledBarcodeWidth = Math.max((width / 3) * 1.5, 0.5);
    const scaledBarcodeHeight = Math.max((height / 2) * 60, 20);
    const scaledQrSize = Math.max(Math.min(width, height) * 150, 100);
    ...
    }, [type, value, format, scaledBarcodeWidth, scaledBarcodeHeight, scaledQrSize]);
...
const TextElement = ({ content, fontSize }) => (
    <div style={{ fontSize: `${fontSize}px`, lineHeight: "1.2" }}>{content}</div>
)
            ...
            <div className="configuration-Qr">
                <div className="qr-maker-container" style={{ display: "flex", gap: "10px" }}>
                    <div className="qr-tools" style={{ minWidth: "150px" }}>
                        ...
                        <Button className="qr-tool-button mb-2" onClick={handleDuplicate}>
                            Dupliquer
                        </Button>
                        <Button className="qr-tool-button mb-2" onClick={handleDuplicate}>
                        ...
                        <div className="qr-edit-field">
                            <div className="dynamic-attributes-container mb-3">
                                ...
                                <div className="dynamic-attributes-grid" style={{ marginTop: "10px" }}>
                                    {DYNAMIC_ATTRIBUTES.filter(a => a.category === state.itemType).map((attr, index) => {
                                        const isSelected = state.elements[state.selectedElement]?.attributes?.some(
                            ...
                            <div className="content-input mb-3">
                                ...
                                <input
                                    ...
                                    style={{marginLeft: "10px", width: "95%", marginBottom: "5px"}}
                                    value={
                                        state.elements[state.selectedElement]?.content ||
                                        state.elements[state.selectedElement]?.value ||
                                        "" // Default to empty string if both content and value are null
                                    }
                                    onChange={e => {
                                        const updatedElements = [...state.elements];
                                        ...
                                        if (selectedElement) {
                                            ...
                                            // Explicitly handle null input values:
                                            if (e.target.value === "") {
                                                selectedElement[key] = null; // Set the key to null for null input
                                            } else {
